...
import { getConferencesForAdmin, updateConferenceStatus } from '../../../actions/conferenceActions';
...
class Conferences extends Component {
  ...
  componentWillReceiveProps = nextProps => {
    if (nextProps.conference !== null) {
      if (this.props.conference !== nextProps.conference) {
        this.setState({ conferences: nextProps.conference }, () => {
          console.log('confer', this.state.conferences)
        });
      }
    }

    if (this.props.setconferencestatus !== nextProps.setconferencestatus) {
      this.props.getConferencesForAdmin();
      NotificationManager.success('Conference is now published');
    }
  }
      ...
      <div>
        <div className="row">
          ...
          <div className="col-md-6">
            ...
            <p><i className="fas fa-clock"></i>&nbsp;{moment(row.resource.time).format('LLLL')}</p>
  ...
  onApproveButtonClick(e, row) {
    ...
    this.props.updateConferenceStatus(conference);
  }
    ...
    return (
      <div className="container p-4">
        <div className="card p-3">
          <h3 className="workshop-title">Conferences</h3>
            <ToolkitProvider
              keyField="_id"
...
const mapStateToProps = state =>({
  ...
  setconferencestatus: state.conferenceReducer.setconferencestatus
});
...
const mapDispatchToProps = dispatch =>({
  ...
  updateConferenceStatus: (conference) => {
    dispatch(updateConferenceStatus(conference));
  }
})
